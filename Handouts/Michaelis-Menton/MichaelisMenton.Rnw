...
\title{\large \sf The Michaelis-Menton Model}
...
\begin{itemize}
	...
	\item
In situations where $\frac{d[P]}{dt}$ is constant (or nearly constant), 
...
The measurements are a bit indirect.  The amount of product is inferred from the 
absorbance, and $v$ is inferred by the rate of change in absorbance, which should be 
a roughly linear function of \variable{time} (in minutes) for each value of $[S]$ 
(\variable{substrate}, mM peroxide) if our assumptions are true.  
...
Now we can fit our Michaelis-Menton model:
